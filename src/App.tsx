...
// ==========================================
// ▼ ここにGoogle Apps ScriptのウェブアプリURLを貼り付けます ▼
// ==========================================
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!GOOGLE_SCRIPT_URL) {
        setError('【連携エラー】スプレッドシートと連携するための「ウェブアプリURL」が設定されていません。チャットの案内をご確認ください。');
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-10 text-center">
          ...
          <p className="text-gray-600 mb-10 leading-relaxed text-sm">
            講座へのお申し込みありがとうございます。<br />
            ご入力いただいた内容を受け付けました。<br />
            ご登録いただいたメールアドレス宛に確認メールをお送りいたしますので、ご確認ください。
          </p>
          <button
            onClick={() => {
        ...
        <form onSubmit={handleSubmit} className="px-6 sm:px-10 py-10 space-y-10">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-colors ${formData.date === '2026年3月29日（日）10:00【宮前市民館】' ? 'border-gray-900 bg-gray-50/50' : 'border-gray-200 hover:bg-gray-50'}`}>
                <input
                  type="radio"
                  name="date"
                  value="2026年3月29日（日）10:00【宮前市民館】"
                  checked={formData.date === '2026年3月29日（日）10:00【宮前市民館】'}
                  onChange={handleChange}
                  className="w-4 h-4 text-gray-900 focus:ring-gray-900 border-gray-300"
                />
                <span className="ml-3 text-gray-900 font-medium">2026年3月29日（日）10:00【宮前市民館】</span>
              </label>
            </div>
          </div>
